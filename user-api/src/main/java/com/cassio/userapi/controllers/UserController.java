 ...
 * Long id) { User user = userService.getUserById(id); if (user != null) {
 * return ResponseEntity.ok(user); } return ResponseEntity.notFound().build(); }
 * 
 * //metodo
 * 
 * // @PostMapping // public @ResponseBody ResponseEntity<User>
 * createUser(@Valid User user) { // User createdUser =
 ...
 * updateUser(@PathVariable Long id, @Valid User updatedUser) { // User updated
 * = userService.updateUser(id, updatedUser); // if (updated != null) { //
 * return ResponseEntity.ok(updated); // } // return
 * ResponseEntity.notFound().build(); // }
 * 
 * @DeleteMapping("/{id}") public ResponseEntity<Void> deleteUser(@PathVariable
 * Long id) { userService.deleteUser(id); return
